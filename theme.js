(function () {
    ...
    var Lamba = {
        ...
        // ... всі інші теми додані нижче в об'єкт values
    };
    ...
    function init() {
        Lampa.Lang.add(Lamba);

        // Створюємо структуру параметра
        var themeParam = {
        ...
        // ФОРСОВАНЕ ДОДАВАННЯ: Слухаємо відкриття розділу Інтерфейс
        Lampa.Settings.listener.follow('open', function (e) {
            if (e.name === 'interface') {
                setTimeout(function() {
                    // Якщо пункт ще не додано в меню, додаємо його
                    if (!$('.settings-param[data-name="interface_theme_custom"]').length) {
                        Lampa.Settings.add(themeParam);
                        // Оновлюємо відображення, щоб пункт з'явився без перезаходу
